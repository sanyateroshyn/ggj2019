...
public class OwlCotroller : MonoBehaviour {
	...
	void FixedUpdate() {
		...
		if (horizontal) {
			if ((Mathf.Abs(panel.position.x - point2.position.x) <= 0.3) || (Mathf.Abs(panel.position.x - point1.position.x) <= 0.3)) {
				right = !right;
				transform.GetComponent<SpriteRenderer>().flipX = !transform.GetComponent<SpriteRenderer>().flipX;
			}

		} else {
			if ((Mathf.Abs(panel.position.y - point2.position.y) <= 0.3) || (Mathf.Abs(panel.position.y - point1.position.y) <= 0.3)) {
				right = !right;
